...
export const getWorkoutDetails = async (date) => {
  // Étape 1: Trouver le workout par date
  const workoutDetails = await prisma.workouts.findFirst({
    where: {
      date,
    },
  ...
  if (workoutDetails) {
    const workoutId = workoutDetails.id;

    const workoutRoundsDetails = await prisma.workoutRounds.findMany({
      where: {
        workoutId,
      },
      select: {
    ...
    workoutDetails.WorkoutRounds = workoutRoundsDetails;
  }
